...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={`fixed inset-0 z-50 bg-black/60 backdrop-blur-[2px] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 duration-fast${className ? ` ${className}` : ""}`}
    {...props}
  />
))
...
>(({ className, children, size = "md", ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={`flex flex-col fixed left-[50%] top-[50%] z-50 w-[calc(100%-2rem)] ${SIZE_MAX[size]} translate-x-[-50%] translate-y-[-50%] gap-0 border border-border bg-surface rounded-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] duration-fast${className ? ` ${className}` : ""}`}
      {...props}
    >
      {children}
      <DialogCloseLabel />
